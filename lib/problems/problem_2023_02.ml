...
module Part_2 = struct
    ...
    let parse_draw num_list str =
      let draw = String.split str ~on:',' in
      List.fold draw ~init:num_list ~f:(fun accum value ->
          handle_draw accum value
        )
    ;;
    ...
    let parse_draws str =
      let draws = String.split str ~on:';' in
      let (red, green, blue) = List.fold draws ~init:([], [], [])  ~f:(fun accum value ->
          parse_draw accum value
        ) in
      let max_in_list lst = match List.max_elt lst ~compare:Int.compare with
      | Some value -> value
      | None -> raise (Error "Empty list") in
    ...
    let run (input : string) : (string, string) result =
       let lines = String.split_lines input in
       let total = List.fold ~init:0 lines ~f:(fun accum value ->
